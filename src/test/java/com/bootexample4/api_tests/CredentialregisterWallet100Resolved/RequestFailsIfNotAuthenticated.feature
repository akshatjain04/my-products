...
Feature: Credential Wallet API
  ...
  Scenario: Request must fail with error if request not authenticated
    # Endpoint: /recipients/persons/{id}/registrations
    # Method: GET
    # Expected Outcome: The request without an authentication token should fail with a 401 status code and the response should have an appropriate error schema.
