...
class PetstoreTest {
	...
	void testAll() {
		...
		String petstoreapikey = System.getenv().get("PETSTORE_API_KEY");
		String petstoreauthtoken = System.getenv().get("PETSTORE_AUTH_TOKEN");
		Results results = Runner.path("src/test/java/com/bootexample4/api_tests/Petstore_f1")
			.systemProperty("PETSTORE_API_HOST", petstoreapihost)
			.systemProperty("PETSTORE_API_KEY", petstoreapikey)
